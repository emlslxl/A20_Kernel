/*
 * Geely EP1000 platform STM32 drivers (SPI bus)
 *
...
#define CMD_RI0				(CMD_DIN_BASE+1)	//RI0, for fuel
#define CMD_PWMI0			(CMD_DIN_BASE+2)	//PWM input
#define CMD_ANALOG_IN(n)	(CMD_RI0+(n))
...
{
	int d;
	d = epstm32_spi_read_word(mcu->spi, CMD_DIN);
	if(d<0){
		printk("%s: failed to get %d\n", __FUNCTION__, n);
		return 0;
	}
...
DEFINE_MCU_AIN(0, RI0)
DEFINE_MCU_AIN(1, PWMI0)
