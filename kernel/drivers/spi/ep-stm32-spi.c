...
#define CMD_DI				(CMD_DIN_BASE+0)	//IO数据输入
#define CMD_PI0				(CMD_DIN_BASE+1)	//PI0
#define CMD_PI1				(CMD_DIN_BASE+2)	//PI1
#define CMD_AI0				(CMD_DIN_BASE+3)	//AI0
#define CMD_AI1				(CMD_DIN_BASE+4)	//AI1
#define CMD_AI2				(CMD_DIN_BASE+5)	//AI2
#define CMD_AI3				(CMD_DIN_BASE+6)	//AI3
#define CMD_RI0				(CMD_DIN_BASE+7)	//RI0
#define CMD_RI1				(CMD_DIN_BASE+8)	//RI1
#define CMD_RI2				(CMD_DIN_BASE+9)	//RI2
#define CMD_RI3				(CMD_DIN_BASE+10)	//RI3
#define CMD_PWMI0			(CMD_DIN_BASE+11)
#define CMD_PWMI1			(CMD_DIN_BASE+12)

#define CMD_ANALOG_IN(n)	(CMD_PI0+(n))
...
#define CMD_PWM0			(CMD_DOUT_BASE+3)	//PWM0输出
#define CMD_PWM1			(CMD_DOUT_BASE+4)	//PWM1输出
#define CMD_METER0			(CMD_DOUT_BASE+5)	//仪表0输出
#define CMD_METER1			(CMD_DOUT_BASE+6)	//仪表1输出
#define CMD_METER2			(CMD_DOUT_BASE+7)	//仪表2输出
...
{
	int d;
	d = epstm32_spi_read_word(mcu->spi, CMD_DOUT);
	if(d<0){
...
static int set_mcu_dout(struct mcu_data *mcu, unsigned int n, unsigned int v)
{
	int d;
	if(v)
		d = epstm32_spi_write_word(mcu->spi, CMD_SETB_DOUT, n);
	else
		d = epstm32_spi_write_word(mcu->spi, CMD_CLRB_DOUT, n);
...
{
	int d;
	d = epstm32_spi_read_word(mcu->spi, CMD_ANALOG_OUT(n));
	if(d<0){
		printk("%s: failed to get %d\n", __FUNCTION__, n);
		return 0;
	}
...
{
	int ret;
	ret = epstm32_spi_write_word(mcu->spi, CMD_ANALOG_OUT(n), v);
	if(ret<0){
		printk("%s: failed to set %d\n", __FUNCTION__, n);
		return ret;
	}
...
DEFINE_MCU_AOUT(0, PWM0)
DEFINE_MCU_AOUT(1, PWM1)
DEFINE_MCU_AOUT(2, METER0)
DEFINE_MCU_AOUT(3, METER1)
DEFINE_MCU_AOUT(4, METER2)

/**********************mcu analog input for PI, AI, RI***************************/
static int get_mcu_ain(struct mcu_data *mcu, unsigned int n)
{
	int d;
	d = epstm32_spi_read_word(mcu->spi, CMD_ANALOG_IN(n));
	if(d<0){
		printk("%s: failed to get %d\n", __FUNCTION__, n);
		return 0;
	}
...
DEFINE_MCU_AIN(0, PI0)
DEFINE_MCU_AIN(1, PI1)
DEFINE_MCU_AIN(2, AI0)
DEFINE_MCU_AIN(3, AI1)
DEFINE_MCU_AIN(4, AI2)
DEFINE_MCU_AIN(5, AI3)
DEFINE_MCU_AIN(6, RI0)
DEFINE_MCU_AIN(7, RI1)
DEFINE_MCU_AIN(8, RI2)
DEFINE_MCU_AIN(9, RI3)
...
static const struct attribute *mcu_attrs[] = {
	...
	&dev_attr_PWM0.attr,
	&dev_attr_PWM1.attr,
	&dev_attr_METER0.attr,
	&dev_attr_METER1.attr,
	&dev_attr_METER2.attr,
...
static irqreturn_t mcu_irq(int irq, void *handle)
{
	...
	while((key = epstm32_spi_read_byte(mcu->spi, CMD_DIN_KEY))!=0xff){
		if(key<0){
