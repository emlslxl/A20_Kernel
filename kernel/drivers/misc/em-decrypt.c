...
#define DECRYPT_DEVICE_NAME	"decrypt"
...
static void AddRoundKey (unsigned long* state, unsigned long *key)
{
	uchar idx;
...
static void ExpandKey (const uchar *key, uchar *expkey)
{
...
static void Decrypt (uchar *in, uchar *expkey, uchar *out)
{
	int idx, round;
	uchar state[Nb * 4];
	...
	AddRoundKey ((unsigned long*)state, (unsigned long*)expkey + Nr * Nb);
	round = Nr;
	...
	while( round-- )
	{
		AddRoundKey ((unsigned long*)state, (unsigned long*)expkey + round * Nb);
		if( round )
...
inline static void AES_init(void)
{
	ExpandKey (kTable, expkey);
}

inline static void aesDecrypt(unsigned char *data)
{
	Decrypt (data, expkey, data);
}

typedef struct{
	unsigned char data[DECRYPT_BLOCK_SIZE];
}decrypt_data;

static ssize_t decrypt_read (struct file *file, 
		char __user *buf, size_t count, loff_t *offset)
{
	decrypt_data *p = (decrypt_data *)file->private_data;

	if(count!=DECRYPT_BLOCK_SIZE)
		return -1;
...
static ssize_t decrypt_write (struct file *file, 
		const char __user *buf, size_t count, loff_t *offset)
{
	...
	copy_from_user(p->data, buf, count);
...
static int decrypt_release(struct inode *inode, struct file *file)
{
...
static const struct file_operations decrypt_fops = {
	...
	.read		= decrypt_read,
	.write		= decrypt_write,
	.release	= decrypt_release,
};

//Initialize driver.
static struct miscdevice misc_decrypt = {
	.minor = MISC_DYNAMIC_MINOR, 
	.name = DECRYPT_DEVICE_NAME, 
	.fops = &decrypt_fops,
};
...
{
	AES_init();
	if(misc_register(&misc_decrypt))
		return -1;

	printk(DECRYPT_DEVICE_NAME " initialized\n");
	return 0;
}
