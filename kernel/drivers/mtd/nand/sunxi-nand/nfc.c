						...
						int page_addr)
{
	...
	case NAND_CMD_READ0:
		...
		dma_nand_config_start(0, (uint32_t)read_buffer, read_size);
		addr_cycle = 5;
		// RAM0 is 1K size
		byte_count =1024;
		wait_rb_flag = 1;
	...
	case NAND_CMD_PAGEPROG:
		...
		//access NFC internal RAM by DMA bus
		writel(readl(NFC_REG_CTL) | NFC_RAM_METHOD, NFC_REG_CTL);
		dma_nand_config_start(1, (uint32_t)write_buffer, write_size);
		// RAM0 is 1K size
		byte_count =1024;
...
void nfc_read_page1k(uint32_t page_addr, void *buff)
{
	...
	writel(readl(NFC_REG_CTL) | NFC_RAM_METHOD, NFC_REG_CTL);
	dma_nand_config_start(0, (uint32_t)buff, 1024);
