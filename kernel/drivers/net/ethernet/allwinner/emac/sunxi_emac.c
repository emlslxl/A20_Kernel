...
void emac_sys_setup(sunxi_emac_board_info_t *db)
{
	...
	if(emac_used){
		if(gpio_request_ex("emac_para", NULL)==0){
			printk(KERN_ERR "ERROR: Request emac gpio resources is failed!\n");
			return;
		}
	}
...
static int gen_mac_by_EFuseID(void)
{
	int i;
	u64 hash=0;
	u32 *sid_key = ioremap(SID_BASE, PAGE_SIZE);

	if(!sid_key)
		return -1;

	for(i=0;i<4;i++){
		hash = BKDRHash(hash, sid_key[i]);
	}
...
unsigned int sunxi_emac_powerup(struct net_device *ndev)
{
	...
	if(!got_mac){
		printk(KERN_WARNING "emac MAC isn't valid! use CPU ID\n");
		if(gen_mac_by_EFuseID()==0)
			got_mac=1;
	}
