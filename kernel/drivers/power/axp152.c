...
#define AXP152_DLDO1_ENABLE	0x02
#define AXP152_ALDO2_ENABLE	0x04
#define AXP152_ALDO1_ENABLE	0x08
...
/* Bits inside AXP152_REG_LDO0 */
#define AXP152_LDO0_ENABLE	(1 << 7)
#define AXP152_LDO0_VOL_NBIT	(2)
#define AXP152_LDO0_VOL_OFFSET	(4)
#define AXP152_LDO0_CUR_NBIT	(2)
#define AXP152_LDO0_CUR_OFFSET	(2)
#define AXP152_LDO0_VOL_MASK	(~(AXP152_LDO0_VOL_NBIT<<AXP152_LDO0_VOL_OFFSET))
#define AXP152_LDO0_CUR_MASK	(~(AXP152_LDO0_CUR_NBIT<<AXP152_LDO0_CUR_OFFSET))
...
int axp_gpio_set_io(int gpio, int io_state){return 0;}
int axp_gpio_get_io(int gpio, int *io_state){return 0;}
int axp_gpio_set_value(int gpio, int value){return 0;}
int axp_gpio_get_value(int gpio, int *value){return 0;}
...
static const int axp152_ldo0_mvolt[4] = { 5000, 3000, 2800, 2500 };

static const int axp152_aldo_mvolt[16] = { 1200, 1300, 1400, 1500, 1600,
	1700, 1800, 1900, 2000, 2500, 2700, 2800, 3000, 3100, 3200, 3300 };
...
static void axp152_irq_work(struct work_struct *work)
{
	...
	if (irqs & (AXP152_IRQ_PEKLONG | AXP152_IRQ_PEKSHORT)) {
		input_report_key(axp152->input, AXP152_POWERKEY, 1);
		input_sync(axp152->input);
		input_report_key(axp152->input, AXP152_POWERKEY, 0);
		input_sync(axp152->input);
	}
...
static int axp152_list_voltage(struct regulator_dev *rdev, unsigned selector)
{
	...
	case axp152_ldo0:
		return axp152_ldo0_mvolt[selector] * 1000;
	case axp152_aldo1:
		return axp152_aldo_mvolt[selector] * 1000;
	case axp152_aldo2:
		return axp152_aldo_mvolt[selector] * 1000;
	case axp152_dldo1:
...
static int axp152_set_voltage(struct regulator_dev *rdev, int val)
{
	...
	case axp152_dldo1: reg = AXP152_REG_DLDO1; break;
	case axp152_dldo2: reg = AXP152_REG_DLDO2; break;
	case axp152_aldo1:
	case axp152_aldo2: reg = AXP152_REG_ALDO12; break;
	default:
		return -EINVAL;
	}

	if (val == -1){
		if(rdev_get_id(rdev)==axp152_aldo1)
			return axp152->regs[reg]>>4;
		else if(rdev_get_id(rdev)==axp152_aldo2)
			return axp152->regs[reg]&0xf;
		else
			return axp152->regs[reg];
	}

	if(rdev_get_id(rdev)==axp152_aldo1)
		val = ((val&0xf)<<4)|(axp152->regs[reg]&0xf);
	else if(rdev_get_id(rdev)==axp152_aldo2)
		val = (val&0xf)|(axp152->regs[reg]&0xf0);
...
static int axp152_set_output(struct regulator_dev *rdev, int val)
{
	...
	case axp152_aldo1: mask = AXP152_ALDO1_ENABLE; break;
	case axp152_aldo2: mask = AXP152_ALDO2_ENABLE; break;
	case axp152_dldo1: mask = AXP152_DLDO1_ENABLE; break;
...
static int axp152_ldo0_set_voltage_sel(struct regulator_dev *rdev,
				  unsigned selector)
{
	struct axp152_data *axp152 = rdev_get_drvdata(rdev);
	int val = ((selector<<AXP152_LDO0_VOL_OFFSET)&AXP152_LDO0_VOL_MASK)
		|(axp152->regs[AXP152_REG_LDO0]&AXP152_LDO0_VOL_MASK);
	return axp152_write_reg(axp152, AXP152_REG_LDO0, val);
}

static int axp152_ldo0_get_voltage_sel(struct regulator_dev *rdev)
{
	struct axp152_data *axp152 = rdev_get_drvdata(rdev);
	return (axp152->regs[AXP152_REG_LDO0]&AXP152_LDO0_VOL_MASK)>>AXP152_LDO0_VOL_OFFSET;
}

static int axp152_ldo0_enable(struct regulator_dev *rdev)
{
	struct axp152_data *axp152 = rdev_get_drvdata(rdev);
	int val = axp152->regs[AXP152_REG_LDO0]|AXP152_LDO0_ENABLE;
	return axp152_write_reg(axp152, AXP152_REG_LDO0, val);
}

static int axp152_ldo0_disable(struct regulator_dev *rdev)
{
	struct axp152_data *axp152 = rdev_get_drvdata(rdev);
	int val = axp152->regs[AXP152_REG_LDO0]&(~AXP152_LDO0_ENABLE);
	return axp152_write_reg(axp152, AXP152_REG_LDO0, val);
...
static int axp152_ldo0_is_enabled(struct regulator_dev *rdev)
{
	struct axp152_data *axp152 = rdev_get_drvdata(rdev);

	return (axp152->regs[AXP152_REG_LDO0]&AXP152_LDO0_ENABLE) ? 1 : 0;
}

static const int axp152_ldo0_current_table[]={2000, 1500, 900, 500};
static int axp152_ldo0_calc_current(struct axp152_data *axp152, unsigned selector)
{
	return axp152_ldo0_current_table[selector] * 1000;
}

static int axp152_ldo0_set_current_limit(struct regulator_dev *rdev,
				 int min_uA, int max_uA)
{
	struct axp152_data *axp152 = rdev_get_drvdata(rdev);
	int sel;

	for(sel=0; sel<ARRAY_SIZE(axp152_ldo0_current_table); sel++){
		if(min_uA<=axp152_ldo0_current_table[sel]){
			if(max_uA < axp152_ldo0_calc_current(axp152, sel))
				return -EINVAL;

			sel = ((sel<<AXP152_LDO0_CUR_OFFSET)&AXP152_LDO0_CUR_MASK)
				|(axp152->regs[AXP152_REG_LDO0]&AXP152_LDO0_CUR_MASK);
			return axp152_write_reg(axp152, AXP152_REG_LDO0, sel);
		}
	}
...
static int axp152_ldo0_get_current_limit(struct regulator_dev *rdev)
{
	struct axp152_data *axp152 = rdev_get_drvdata(rdev);
	return axp152_ldo0_calc_current(axp152, 
		(axp152->regs[AXP152_REG_LDO0]&AXP152_LDO0_CUR_MASK)>>AXP152_LDO0_CUR_OFFSET);
}

static struct regulator_ops axp152_ldo0_ops = {
	.list_voltage		= axp152_list_voltage,
	.set_voltage_sel	= axp152_ldo0_set_voltage_sel,
	.get_voltage_sel	= axp152_ldo0_get_voltage_sel,
	.set_current_limit	= axp152_ldo0_set_current_limit,
	.get_current_limit	= axp152_ldo0_get_current_limit,
	.enable			= axp152_ldo0_enable,
	.disable		= axp152_ldo0_disable,
	.is_enabled		= axp152_ldo0_is_enabled,
};
				  ...
				  const struct i2c_device_id *id)
{
	...
	ret |= axp152_read_reg(axp152, AXP152_REG_LDO0);
	ret |= axp152_read_reg(axp152, AXP152_REG_ALDO12);
	ret |= axp152_read_reg(axp152, AXP152_REG_DLDO1);
	...
	if (client->irq != -1) {
		...
		set_bit(AXP152_POWERKEY, axp152->input->keybit);
...
static struct regulator_init_data regl_init_data[AXP152_REGULATOR_COUNT] = {
	...
	[axp152_ldo0] = { /* Power on 5.0V */
		.num_consumer_supplies = 1,
		.consumer_supplies = &axp152_ldo0_supply,
		.constraints = {
			.min_uV =  5000 * 1000,
			.max_uV =  5000 * 1000,
			.always_on = 1,
	...
	[axp152_aldo1] = { /* Power on 3.0V,  3.0V for AVCC*/
		.num_consumer_supplies = 1,
		.consumer_supplies = &axp152_aldo1_supply,
		.constraints = {
			.min_uV =  3000 * 1000,
			.max_uV =  3000 * 1000,
			.always_on = 1,
	...
	[axp152_aldo2] = { /* Power on 1.2V */
		.num_consumer_supplies = 1,
		.consumer_supplies = &axp152_aldo2_supply,
		.constraints = {
			.min_uV =  1200 * 1000,
			.max_uV =  3300 * 1000,
			.always_on = 1,
	...
	[axp152_dldo1] = { /* Power on x.xV,  2.8V for CSI*/
		.num_consumer_supplies = 1,
		.consumer_supplies = &axp152_dldo1_supply,
		.constraints = {
			.min_uV =  2800 * 1000,
			.max_uV =  2800 * 1000,
			.always_on = 1,
...
static int __init axp_board_init(void)
{
	...
	#define set_script_voltage(channel)	\
		ret = script_parser_fetch("target", #channel"_vol", &val, sizeof(int));\
		if (ret == 0) {\
			regl_init_data[axp152_##channel].constraints.min_uV = val * 1000;\
			regl_init_data[axp152_##channel].constraints.max_uV = val * 1000;\
		}

	set_script_voltage(dcdc3);
	set_script_voltage(dcdc4);
	set_script_voltage(ldo0);
	set_script_voltage(aldo1);
	set_script_voltage(aldo2);
	set_script_voltage(dldo1);
	set_script_voltage(dldo2);
