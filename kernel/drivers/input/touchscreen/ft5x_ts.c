...
static int get_gpio_irq(void)
{
    unsigned int irq_gpio = 0;
	int err;

	err = script_parser_fetch("ctp_para", "irq_gpio", (void*)&irq_gpio, sizeof(unsigned int));
	if (err != SCRIPT_PARSER_OK) {
		pr_info("can't get ctp_para irq gpio\n");
		return -1;
	}
	if(gpio_request(irq_gpio, "ctp_para") == 0)
		return gpio_to_irq(irq_gpio);
...
//	pr_info("==INIT_WORK=\n");
	...
	err = request_irq(err, ft5x_ts_interrupt, IRQF_TRIGGER_FALLING | IRQF_SHARED, "ft5x_ts", ft5x_ts);
