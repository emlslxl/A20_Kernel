...
#ifdef CONFIG_SUPPORT_ROOT2
#include <linux/namei.h>

static char root2_line[128], *proot2;
...
static struct root2_info last_root2_info;

/*
	format: root2=[wait time]:[device],[filesystem],[initscript];[device],[filesystem],[initscript]
	example: 
		root2=20:/dev/mtdblock4,squashfs,/linurc
*/
static int get_root2_info(struct root2_info *info)
{
	char *s=proot2;
	...
	if((((u32)s)-((u32)root2_line))>sizeof(root2_line)){	//first use
		strncpy(root2_line, s, sizeof(root2_line));
		s = proot2 = root2_line;
	}

	memset(info, 0, sizeof(struct root2_info));
	...
	for(;;s++){
		...
		if (*s == ';' || *s == 0){	//end
			*s++ = 0;
			proot2 = s;
			printk(KERN_WARNING"error root2 format at device name\n");
			return -1;
		}
	}
	...
	for(;;s++){
		...
		if (*s == ';' || *s == 0){	//end
			*s++ = 0;
			proot2 = s;
			printk(KERN_WARNING"error root2 format at fs name\n");
			return -1;
		}
	}
	...
	memcpy(&last_root2_info, info, sizeof(last_root2_info));
	proot2 = s;
...
static int __init do_initroot(char *init_filename)
{
	...
	return kernel_execve(init_filename, (const char *const*)argv, (const char *const*)envp_init);
}
...
{
	struct root2_info info;
	int pid, retv=0;

	retv=get_root2_info(&info);
	if(retv)
		goto end;

	create_dev("/dev/initroot", name_to_dev_t(info.dev_name));
